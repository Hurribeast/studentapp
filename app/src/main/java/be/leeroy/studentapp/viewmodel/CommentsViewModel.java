...
public class CommentsViewModel extends AndroidViewModel {
    ...
    private final MutableLiveData<Boolean> _commented = new MutableLiveData<>();
    private final LiveData<Boolean> commented = _commented;
    ...
    public void addComment(String content, Integer publiId, String token) {
        HashMap<String, Object> body = new HashMap<>();
        body.put("content", content);
        body.put("publiId", publiId);

        RequestBody requestBody = ApiUtils.ToRequestBody(body);
        publicationDataAccess.createComment(token, requestBody).enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(@NonNull Call<ResponseBody> call, @NonNull Response<ResponseBody> response) {
                if(response.isSuccessful()) {
                    _commented.setValue(true);
                    _error.setValue(null);
                } else if(response.code() == 401) {
                    _error.setValue(Errors.TOKEN_EXPIRED);
                } else {
    ...
    public LiveData<Boolean> isCommented(){
        return commented;
    }

    public void setNotCommented(){
        _commented.setValue(false);
    }
